...
- Debug sample data keeps its root `DemoDataSeeder` as a build-variant facade only; debug Room writes belong to `data/local/DebugDemoDataSeeder` and run as one `DatabaseTransactionRunner` transaction, while release remains a no-op
