...
- Voice commands v2: `VoiceCommandHandler` (continuous + one-shot, sealed class `VoiceCommand` with count), `VoiceCommandInterpreter` (Gemini AI fallback), `VoiceResponseManager` (TTS), `AiQuotaManager.hasVoiceQuota()` jakaa 500/kk AI-kiintiön. Tunnistus: exact keyword → counted command (numerot 1-100, sanaluvut useilla localeilla) → first-word fallback → Gemini
