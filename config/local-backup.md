...
The mobile acceptance budget is shared by export, preview validation, confirmation-time revalidation, shadow import and live replacement. It allows at most 2,019 ZIP entries including the manifest, 2,000 durable files, 200 MiB per durable file, 448 MiB of durable file bytes, 512 MiB for both the archive and expanded payload, 32 MiB per table, 64 MiB across all tables, 100,000 rows per table, 250,000 rows in total, 2 Mi UTF-16 units per JSON line, 256 Ki UTF-16 units per text field, and a 512 KiB manifest. Restore may retain at most 100,000 identity mappings per table and 150,000 in total; dangling chart-counter identities consume the same budget. These values follow the app's existing 200 MiB pattern image/PDF output precedent and the 256 KiB annotation/row-mapping field precedent while leaving ample room for ordinary mobile project history. Yarn photos retain their stricter 25 MiB source limit.

A bounded structural pass checks classic/ZIP64 directory counts, offsets and headers before opening `ZipFile`; the central directory is limited to 1,033,728 bytes. Extraction enforces declared per-entry and aggregate sizes while streaming, before accepting hashes. Table row, field, file-copy and identity totals use overflow-safe `Long` arithmetic. JSON nesting is bounded before parsing. Over-budget tables and file plans are rejected before shadow-database insertion, preview publication or live mutation, and direct identity-map construction enforces the same mapping budget without a set-then-map copy. The app preserves a 32 MiB free-space reserve and estimates shadow/replacement database space separately; it may need several temporary copies of the content. Insufficient space aborts without replacing live data.
